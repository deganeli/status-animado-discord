//META{"name":"StatusAnimado","":"https://github.com/r4zor2k/status-animado-discord","siaj":"https://github.com/r4zor2k/status-animado-discord"}*//
...
class AnimatedStatus {
	...
	getDescription() {
		return "Troque seu status personalizado de forma automática.";
	}
	...
	Status_Animate(index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("Status Animado: Sem status selecionado. Va até Settings>Plugins para personalizar!");
			return;
		}
	...
	newRawEdit(str = "") {
		...
		out.placeholder = '"Teste (Mensagem)"\n"Teste (Mensagem)", "👍 (Símbolo)"\n"Teste (Mensagem)", "emoji (Emoji Nitro)", "000000000000000000 (ID Emoji Nitro)"\n"eval new String(\'test\') (Javascript)"\n"eval new String(\'test\') (Javascript)", "eval new String(\'👍\') (Javascript)"\n...';
		out.value = str;
		return out;
	}
	...
	newRichRow(text, emoji, optNitroId = undefined) {
		...
		emojiWidget.placeholder = "nome_emoji_nitro";
		...
		optNitroIdWidget.placeholder = "id_emoji_nitro";
	...
	getSettingsPanel() {
		...
		// timeout
		settings.appendChild(GUI.newLabel("TEMPO PARA MUDAR STATUS (Milissegundos)"));
		...
		// Animation
		settings.appendChild(GUI.newLabel('Animação'));
		...
		let changeEditMode = GUI.newButton("Alterar modo de edição");
	...
	strerror: (req) => {
		...
		for (const s of ["errors", "custom_status", "text", "_errors", 0, "message"])
			if ((json == undefined) || ((json = json[s]) == undefined))
				return "Internal. Report at github.com/H20/BetterDiscord-Animated-Status";
